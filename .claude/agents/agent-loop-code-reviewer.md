...
Review only the supplied diff, branch, or files **plus the task/brief evidence** that defines scope and acceptance criteria. Inspect the complete raw diff and real source; use `tools/agent-loop/vendor/bin/agent-loop map changed --base=<ref>` plus focused caller/context lookup when needed.
